...
(function($) {
    ...
    class DreddChat {
        ...
        generateSessionId() {
            return 'dredd_' + Math.random().toString(36).substr(2, 9) + '_' + Date.now();
        }
        ...
        bindEvents() {
            ...
            $(document).on('click', '.payment-modal-close', () => this.closePaymentModal());
            ...
            $('#connect-pulsechain-wallet').on('click', () => this.connectPulseChainWallet());
            $('#pulsechain-submit-btn').on('click', () => this.processPulseChainPayment());
            
            // Copy buttons
            $(document).on('click', '.copy-address-btn, .copy-amount-btn', (e) => this.copyToClipboard(e));
            ...
            $(document).on('click', '.dashboard-modal-close', () => this.closeDashboardModal());
            ...
            $(document).on('click', '.dashboard-export-data', (e) => this.exportUserData(e));
            $(document).on('click', '.dashboard-view-analysis', (e) => this.viewAnalysis(e));
            $(document).on('click', '.dashboard-reanalyze', (e) => this.reanalyzeToken(e));
            $(document).on('click', '.dashboard-submit-promotion', (e) => this.submitPromotion(e));
            $(document).on('submit', '.dashboard-settings-form', (e) => this.updateUserSettings(e));
            $(document).on('submit', '.dashboard-password-form', (e) => this.updateUserPassword(e));
            ...
            $(document).on('input keyup focus click', '.form-group input[type="text"], .form-group input[type="email"], .form-group input[type="password"]', function() {
                $(this).css({
        ...
        activateMode(mode) {
            ...
            $('.dredd-chat-header').removeClass('standard-mode psycho-mode').addClass(`${mode}-mode`);
            
            // Show mode activation message
            let modeMessage;
            if (mode === 'psycho') {
                if (dredd_ajax.paid_mode_enabled === 'true') {
                    modeMessage = '💀 PSYCHO MODE ACTIVATED - Brutal honesty unlocked! Credits will be deducted per analysis.';
                } else {
        ...
        changeChain(e) {
            ...
            const chainMessage = `🔗 Blockchain switched to ${chainNames[newChain]}. Ready to investigate tokens on this chain!`;
        ...
        handleResponse(response) {
            ...
            if (response.success) {
                ...
                if (response.data) {
                    ...
                    console.log('DREDD Frontend - Message Length:', response.data.message ? response.data.message.length : 'N/A');
                }
            } else {
            ...
            if (response.success) {
                ...
                if (data.action === 'response' && data.message) {
                    ...
                    if (data.message.includes('test') || data.message.includes('TEST')) {
                        alert('DEBUG: Response received! Message starts with: ' + data.message.substring(0, 50) + '...');
                    }
                ...
                switch (data.action) {
                    ...
                    default:
                        ...
                        if (data.message) {
                            ...
                            this.addMessage('Analysis completed but response format was unexpected.', 'dredd', 'warning');
                        }
                        break;
                }
            } else {
        ...
        formatMessage(message) {
            ...
            return message
                ...
                .replace(/\*(.*?)\*/g, '<em>$1</em>')
                .replace(/GUILTY|SCAM|DANGER|CRIMINAL/gi, '<span class="danger">$&</span>')
                .replace(/LEGIT|SAFE|APPROVED|INNOCENT/gi, '<span class="safe">$&</span>')
                .replace(/I AM THE LAW/gi, '<span class="law">$&</span>')
                .replace(/CAUTION|WARNING|SUSPICIOUS/gi, '<span class="warning">$&</span>');
        }
        ...
        togglePromotions() {
            ...
            if ($sidebar.length) {
                ...
                if (window.console) {
                    ...
                    console.log('DREDD: Sidebar is now', $sidebar.hasClass('open') ? 'OPEN' : 'CLOSED');
                    console.log('DREDD: Sidebar content length:', $sidebar.find('.promotions-content').html().length, 'chars');
                    console.log('DREDD: Sidebar HTML:', $sidebar.find('.promotions-content').html());
                }
            } else {
        ...
        loadPromotions() {
            ...
            $('.analyze-promoted, .read-more').on('click', function() {
                const promotionId = $(this).closest('.promotion-card').data('promotion-id');
                if (promotionId) {
                    $.ajax({
                        url: dredd_ajax.ajax_url,
                        type: 'POST',
        ...
        reanalyzeToken(e) {
            ...
            window.location.href = `${window.location.origin}?dredd_message=${encodeURIComponent(message)}`;
        }
        ...
        exportUserData(e) {
            ...
            if (confirm('Export all your DREDD AI data? This may take a moment.')) {
                window.location.href = dredd_ajax.ajax_url + '?action=dredd_export_user_data&nonce=' + dredd_ajax.nonce;
            }
        }
                    ...
                    <div class="dashboard-section active" id="overview-section">
                        <div class="stats-grid">
                            <div class="stat-card">
                                ...
                                <div class="stat-value">${data.user.credits || 0}</div>
                                <div class="stat-label">Credits</div>
                            </div>
                            <div class="stat-card">
                                <div class="stat-icon">📈</div>
                                <div class="stat-value">${data.stats.total_analyses || 0}</div>
                                <div class="stat-label">Analyses</div>
                            </div>
                            <div class="stat-card">
                                <div class="stat-icon">🚨</div>
                                <div class="stat-value">${data.stats.scams_detected || 0}</div>
                                <div class="stat-label">Scams Detected</div>
                            </div>
                            <div class="stat-card">
                                <div class="stat-icon">💀</div>
                                <div class="stat-value">${data.stats.psycho_analyses || 0}</div>
                        ...
                        <div class="quick-actions-grid">
                            ...
                            <button class="action-card" onclick="window.open('${window.location.origin}', '_blank')">
                                <div class="action-icon">⚡</div>
                                <div class="action-title">New Analysis</div>
                                <div class="action-desc">Investigate tokens</div>
                            </button>
                        </div>
                    </div>
                            ...
                            <div class="settings-panel active" id="profile-panel">
                                ...
                                <form class="dashboard-settings-form">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="display_name" value="${data.user.display_name || ''}" required>
                                    </div>
                                    <div class="form-group">
            ...
            return history.map(item => `
                <div class="history-item">
                    <div class="history-token">
                        <div class="token-name">${item.token_name || 'Unknown Token'}</div>
                        <div class="token-address">${item.contract_address ? item.contract_address.substring(0, 10) + '...' : 'N/A'}</div>
                    </div>
                    ...
                    <div class="history-actions">
                        <button class="history-btn dashboard-view-analysis" data-id="${item.analysis_id}">🔍 View</button>
                        <button class="history-btn dashboard-reanalyze" data-contract="${item.contract_address}" data-chain="${item.chain}">♾️ Re-analyze</button>
                    </div>
                </div>
            `).join('');
        }
        ...
        submitPromotion(e) {
            ...
            $.ajax({
                ...
                success: (response) => {
                    if (response.success) {
                        this.showMessage('Promotion submitted successfully! It will be reviewed by our team.', 'success');
                        $form[0].reset();
                    } else {
                        this.showMessage('Failed to submit promotion: ' + response.data, 'error');
                    }
                },
                error: () => {
            ...
            $.ajax({
                ...
                success: (response) => {
                    if (response.success) {
                        ...
                        this.showMessage('Failed to update profile: ' + response.data, 'error');
                    }
                },
                error: () => {
                    this.showMessage('Network error. Please try again.', 'error');
            ...
            $.ajax({
                ...
                success: (response) => {
                    if (response.success) {
                        ...
                        this.showMessage('Failed to update password: ' + response.data, 'error');
                    }
                },
                error: () => {
                    this.showMessage('Network error. Please try again.', 'error');
        ...
        initStripe() {
            if (typeof Stripe === 'undefined') {
                ...
                $('#stripe-submit').prop('disabled', true).text('STRIPE NOT CONFIGURED');
                return;
            }
        ...
        processStripePayment() {
            ...
            $.ajax({
                ...
                success: (response) => {
                    if (response.success) {
                        this.stripe.confirmCardPayment(response.data.client_secret, {
                            payment_method: {
        ...
        handlePaymentSuccess(paymentIntent) {
            ...
            this.addMessage(`✅ Payment successful! ${this.selectedPackage.tokens} tokens added to your account.`, 'dredd', 'success');
        ...
        initWeb3() {
            ...
            $('#connect-walletconnect').on('click', () => this.connectWalletConnect());
        ...
        async connectMetaMask() {
            ...
            try {
                ...
                $('#connect-metamask').text('Connected: ' + this.userAccount.substr(0, 6) + '...' + this.userAccount.substr(-4));
        ...
        initStripeElements() {
            if (!this.stripeCardElement) {
                if (typeof Stripe !== 'undefined' && dredd_ajax.stripe_publishable_key) {
                    this.stripe = Stripe(dredd_ajax.stripe_publishable_key);
                    this.elements = this.stripe.elements();
        ...
        createCryptoPayment() {
            ...
            console.log('Creating crypto payment for:', this.selectedPaymentMethod, 'Amount:', this.selectedAmount);
            ...
            $.ajax({
                ...
                success: (response) => {
                    if (response.success) {
                        ...
                        if (errorMsg.includes('address') || errorMsg.includes('configured')) {
                        ...
                        console.error('Payment setup failed - no fallback to demo/test system');
                    }
                },
                error: (xhr, status, error) => {
                    // 🚨 AJAX ERROR
                    console.error('AJAX payment error:', xhr, status, error);
                    alert('🚨 Connection Error: Could not connect to payment system. Please try again.');
        ...
        displayCryptoPaymentInfo(paymentData) {
            ...
            if (!paymentData.payment_address || paymentData.payment_address === null || paymentData.payment_address.trim() === '') {
                ...
                // Show error message
                alert('🚨 Payment Error: No wallet address configured for ' + (paymentData.currency || 'this currency') + '. Please contact administrator.');
                return;
            }
            ...
            $('#crypto-amount').val(paymentData.payment_amount + ' ' + paymentData.currency);
            
            console.log('✅ Address successfully displayed:', paymentData.payment_address);
            ...
            if (paymentData.qr_code) {
                ...
                const demoQrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=100x100&data=${encodeURIComponent(paymentData.payment_address)}`;
        ...
        startPaymentTimer() {
            ...
            this.paymentTimer = setInterval(() => {
                ...
                $('#payment-timer').text(`${minutes}:${seconds.toString().padStart(2, '0')}`);
        ...
        processPulseChainPayment() {
            ...
            $.ajax({
                ...
                success: (response) => {
                    if (response.success) {
                        this.sendPulseChainTransaction(response.data).then((txHash) => {
                            // Verify payment
        ...
        handlePaymentSuccess(method) {
            this.closePaymentModal();
            this.addMessage(`✅ Payment successful via ${method}! Credits added to your account.`, 'dredd', 'success');
        ...
        showAuthModal(formType = 'login') {
            ...
            $('.form-group input[type="text"], .form-group input[type="email"], .form-group input[type="password"]').css({
                'color': '#FFFFFF !important',
                'caret-color': '#00FFFF !important',
                '-webkit-text-fill-color': '#FFFFFF !important'
            });
        ...
        handleLogin() {
            ...
            // Show loading state
            $submitBtn.prop('disabled', true).html('<span class="btn-icon">⏳</span>AUTHENTICATING...');
            
            // Send AJAX request
            $.post(dredd_ajax.ajax_url, formData)
                .done((response) => {
                    if (response.success) {
                        this.showMessage('Login successful! Welcome back, citizen!', 'success');
                        this.closeAuthModal();
                ...
                .always(() => {
                    $submitBtn.prop('disabled', false).html('<span class="btn-icon">⚔️</span>ENTER THE SYSTEM');
                });
        }
        ...
        handleSignup() {
            ...
            let recaptchaResponse = '';
            if (typeof grecaptcha !== 'undefined') {
                recaptchaResponse = grecaptcha.getResponse(window.signupRecaptchaWidget);
            }
            ...
            // Show loading state
            $submitBtn.prop('disabled', true).html('<span class="btn-icon">⏳</span>JOINING FORCE...');
            ...
            $.post(dredd_ajax.ajax_url, formData)
                .done((response) => {
                    if (response.success) {
                        ...
                        this.showAuthError('signup', response.data || 'Registration failed');
                    }
                })
                .fail(() => {
                ...
                .always(() => {
                    $submitBtn.prop('disabled', false).html('<span class="btn-icon">🔥</span>JOIN THE FORCE');
                });
        }
        ...
        handleForgotPassword() {
            ...
            // Show loading state
            $submitBtn.prop('disabled', true).html('<span class="btn-icon">⏳</span>SENDING...');
            ...
            $.post(dredd_ajax.ajax_url, formData)
                ...
                .always(() => {
                    $submitBtn.prop('disabled', false).html('<span class="btn-icon">📨</span>SEND RESET LINK');
                });
        }
        ...
        checkForNotifications() {
            ...
            if (urlParams.get('email_verified') === '1') {
                ...
                this.showMessage('Email verified successfully! Welcome to DREDD AI. You can now log in.', 'success');
                this.addMessage('✨ Email verified successfully! Welcome to DREDD AI. You can now log in and start analyzing tokens!', 'dredd', 'success');
                shouldCleanUrl = true;
            }
            ...
            if (urlParams.get('password_reset') === '1') {
                ...
                this.addMessage('🔑 Password reset successfully! You can now log in with your new password.', 'dredd', 'success');
                shouldCleanUrl = true;
            }
            ...
            if (shouldCleanUrl) {
                this.cleanUrlParameters(['email_verified', 'password_reset', 'reset_error']);
            }
        }
        ...
        fixInputVisibility() {
            ...
            const inputSelector = '.form-group input[type="text"], .form-group input[type="email"], .form-group input[type="password"]';
        ...
        initTouchSupport() {
            ...
            $(document).on('touchend', '.auth-btn, .user-menu-btn, .mode-btn, .payment-method-card, .amount-option', function(e) {
                e.preventDefault();
                $(this).trigger('click');
            });
            
            // Prevent double-tap zoom on buttons
            $(document).on('touchend', 'button, .btn, .auth-btn, .payment-continue-btn', function(e) {
                e.preventDefault();
            });
        ...
        adjustModalsForMobile() {
            ...
            $(document).on('show.bs.modal', '.dredd-auth-modal, .dredd-payment-modal', function() {
                $('body').css('overflow', 'hidden');
            });
            
            $(document).on('hide.bs.modal', '.dredd-auth-modal, .dredd-payment-modal', function() {
                $('body').css('overflow', 'auto');
            });
            
            // Close modal when clicking overlay on mobile
            $(document).on('touchend', '.auth-modal-overlay, .payment-modal-overlay', function(e) {
                if (e.target === e.currentTarget) {
                    $(this).closest('.dredd-auth-modal, .dredd-payment-modal').fadeOut(300);
                }
            });
        }
        ...
        handleResize() {
            ...
            if ($('.dredd-auth-modal:visible, .dredd-payment-modal:visible').length > 0) {
        ...
        forceModalRerender() {
            ...
            const $visibleModals = $('.dredd-auth-modal:visible, .dredd-payment-modal:visible');
    ...
    $(document).ready(function() {
        ...
        setTimeout(() => {
            ...
            if ($('.login-btn, .signup-btn').length > 0) {
                ...
                $('.login-btn').off('click.auth').on('click.auth', function(e) {
                ...
                $('.signup-btn').off('click.auth').on('click.auth', function(e) {
                ...
                if ($chatInput.length) {
                    ...
                    setTimeout(() => {
                        ...
                        window.dreddChat.addMessage(
                            '🔑 Password reset complete! Welcome back, citizen. Ready to investigate some tokens?', 
                            'dredd', 
                            'success'
                        );
    ...
    function checkForUserUpdates() {
        ...
        window.dreddChat.checkUserCredits().then((currentCredits) => {
            if (lastKnownCredits !== null && lastKnownCredits !== currentCredits) {
                console.log(`💰 Credits changed: ${lastKnownCredits} → ${currentCredits}`);
        ...
        if (window.dreddChat && window.dreddChat.showMessage) {
            window.dreddChat.showMessage(
                `${emoji} Admin Update: ${Math.abs(difference)} credits ${action} to your account!`,
                'dredd',
                'info'
            );
        } else {
