...
(function($) {
    ...
    class DreddAdmin {
        ...
        bindEvents() {
            ...
            $(document).on('click', '.remove-package', (e) => this.removeTokenPackage(e));
            
            // Promotion management - using event delegation for dynamic content
            $(document).on('click', '.edit-promotion', (e) => this.editPromotion(e));
            $(document).on('click', '.approve-promotion', (e) => this.approvePromotion(e));
            $(document).on('click', '.cancel-promotion', (e) => this.cancelPromotion(e));
            $(document).on('click', '.delete-promotion', (e) => this.deletePromotion(e));
            ...
            $.ajax({
                ...
                success: (response) => {
                    if (response.success) {
                        ...
                        $btn.closest('.status-item').find('.status-value')
                            .removeClass('offline').addClass('online').text('Online');
                    } else {
                        this.showNotice('Connection failed: ' + response.message, 'error');
                        $btn.closest('.status-item').find('.status-value')
                            .removeClass('online').addClass('offline').text('Offline');
                    }
                },
                error: () => {
        ...
        togglePaidMode(e) {
            ...
            if (!confirm(`Are you sure you want to ${currentMode ? 'enable' : 'disable'} paid mode?`)) {
                return;
            }
            ...
            $.ajax({
                ...
                success: (response) => {
                    if (response.success) {
                        ...
                        this.showNotice(`Paid mode ${currentMode ? 'enabled' : 'disabled'}`, 'success');
        ...
        clearCache(e) {
            ...
            if (!confirm('Are you sure you want to clear all cached analysis data?')) {
                return;
            }
        ...
        exportData(e) {
            ...
            const dateFrom = prompt('Start date (YYYY-MM-DD):', this.getDateWeeksAgo(4));
            ...
            const downloadUrl = dredd_admin_ajax.ajax_url + 
        ...
        editPromotion(e) {
            ...
            if (status === 'active') {
                if (!confirm(`Are you sure you want to pause the active promotion "${tokenName}"?`)) {
                    return;
                }
                    ...
                    success: (response) => {
                        if (response.success) {
                            $row.find('.status-badge')
                                .removeClass('active').addClass('cancelled').text('Cancelled');
...
2. Create a new promotion with updated details`);
                ...
                success: (response) => {
                    if (response.success) {
                        $btn.closest('tr').find('.status-badge')
                            .removeClass('pending').addClass('active').text('Active');
                        $btn.remove();
        ...
        cancelPromotion(e) {
            ...
            $.ajax({
                ...
                success: (response) => {
                    if (response.success) {
                        $btn.closest('tr').find('.status-badge')
                            .removeClass('active pending').addClass('cancelled').text('Cancelled');
        ...
        deletePromotion(e) {
            ...
            if (!confirm(`Are you sure you want to permanently delete the promotion "${tokenName}"? This action cannot be undone.`)) {
                return;
            }
            ...
            $.ajax({
                ...
                success: (response) => {
                    if (response.success) {
                        ...
                        this.showNotice('Failed to delete promotion: ' + response.data, 'error');
                    }
                },
                error: () => {
        ...
        saveSettings(e) {
            ...
            $.ajax({
                url: dredd_admin_ajax.ajax_url,
                type: 'POST',
                data: $form.serialize() + '&action=dredd_save_settings&nonce=' + dredd_admin_ajax.nonce,
                success: (response) => {
                    if (response.success) {
        ...
        addPromotion(e) {
            ...
            const contractAddress = $form.find('input[name="contract_address"]').val().trim();
            ...
            if (tokenName.length > 100) {
                this.showNotice('Token name is too long (maximum 100 characters)', 'error');
                return;
            }
            
            if (tokenSymbol.length > 20) {
                this.showNotice('Token symbol is too long (maximum 20 characters)', 'error');
                return;
            }
            
            if (contractAddress.length > 42) {
                this.showNotice('Contract address is too long (maximum 42 characters)', 'error');
                return;
            }
            
            if (tagline.length > 255) {
                this.showNotice('Tagline is too long (maximum 255 characters)', 'error');
                return;
            }
            ...
            $.ajax({
                url: dredd_admin_ajax.ajax_url,
                type: 'POST',
                data: $form.serialize() + '&action=dredd_add_promotion&nonce=' + dredd_admin_ajax.nonce,
                success: (response) => {
                    if (response.success) {
                        ...
                        this.showNotice('Failed to add promotion: ' + response.data, 'error');
                    }
                },
                error: () => {
            ...
            if (stats.analyses_24h !== undefined) {
                $('.stat-card').each(function() {
                    ...
                    if (title.includes('analyses')) {
                        ...
                        $card.find('.stat-number').text('$' + (stats.revenue_24h || 0).toFixed(2));
    ...
    $(document).ready(function() {
        ...
        setInterval(() => {
            if (window.dreddAdmin && typeof window.dreddAdmin.loadDashboardData === 'function') {
