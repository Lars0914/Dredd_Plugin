...
class DreddAI {
    ...
    private function load_dependencies() {
        ...
        require_once DREDD_AI_PLUGIN_DIR . 'includes/class-dredd-wallet.php';
        require_once DREDD_AI_PLUGIN_DIR . 'includes/class-dredd-crypto.php';
    ...
    public function enqueue_admin_assets($hook) {
        if (strpos($hook, 'dredd-ai') === false) return;
    ...
    public function handle_view_logs() {
            ...
            if (file_exists($log_file)) {
                ...
                foreach (array_reverse($lines) as $line) {
                    if (strpos($line, 'DREDD') !== false) {
                        $dredd_lines[] = $line;
                        if (count($dredd_lines) >= 100) break;
                    }
                }
        ...
        <head>
            ...
            <style>
                body { font-family: 'Poppins', monospace; background: #000; color: #0f0; padding: 20px; }
                .log-entry { margin: 5px 0; padding: 5px; background: #111; border-left: 3px solid #0f0; }
                .error { border-left-color: #f00; color: #f88; }
                .warning { border-left-color: #fa0; color: #ffa; }
                .debug { border-left-color: #08f; color: #8cf; }
            </style>
        </head>
        <body>
    ...
    public function get_user_data() {
        ...
        $database = new Dredd_Database();
        $user_data = $database->get_user_data(get_current_user_id());
    ...
    public function handle_debug_payment_flow() {
        ...
        $frontend_currencies = array(
            ...
            'tether-trc20', 'tether-erc20', 'tether-bep20',
            'usdcoin', 'pulsechain'
        );
        ...
        wp_send_json_success(array(
            ...
            'summary' => array(
                ...
                'currency_mapping_good' => !array_filter($mapping_results, function($r) { return strpos($r['normalized'], '-') !== false; }),
    ...
    public function handle_register() {
        ...
        // Store user data in chat users table FIRST
        global $wpdb;
        $chat_users_table = $wpdb->prefix . 'dredd_chat_users';
        ...
        // Check if username or email already exists in our table
        ...
        // Create WordPress user (but don't send any emails)
        add_filter('wp_mail', '__return_false'); // Prevent any emails from being sent
